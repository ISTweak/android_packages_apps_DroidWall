...
public final class Api {
	...
	private static final String BOOT_FILE = "iptables.sh";
	...
	private static String scriptHeader(Context ctx) {
		...
		if ( new File("/data/root/bin/grep").exists() ) {
			grep = "/data/root/bin/grep";
		} else if ( new File("/data/local/bin/grep").exists() ) {
			grep = "/data/local/bin/grep";
		}
		
		String iptables = dir + "/iptables_armv5";
		if ( new File("/data/root/bin/iptables").exists() ) {
			iptables = "/data/root/bin/iptables";
		} else if ( new File("/data/local/bin/iptables").exists() ) {
			iptables = "/data/local/bin/iptables";
		} else if ( new File("/system/bin/iptables2").exists() ) {
			iptables = "/system/bin/iptables2";
		} else if ( new File("/system/bin/iptables").exists() ) {
	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, List<Integer> uidsWimax, boolean showErrors) {
		...
		try {
			...
			code = runScriptAsRoot2(ctx, script.toString(), res);
	...
	public static int runScript2(Context ctx, String script, StringBuilder res, long timeout, boolean asroot) {
		final File file = new File(ctx.getDir("bin", 0), BOOT_FILE);
		final ScriptRunner runner = new ScriptRunner(file, script, res, asroot);
		runner.start();
		try {
			if (timeout > 0) {
	...
	public static int runScriptAsRoot2(Context ctx, String script, StringBuilder res, long timeout) {
		return runScript2(ctx, script, res, timeout, true);
    }
	
	public static int runScriptAsRoot2(Context ctx, String script, StringBuilder res) throws IOException {
		return runScriptAsRoot2(ctx, script, res, 40000);
	}
    /**
		...
		public void run() {
			try {
				...
				if (this.asroot) {
					...
					if ( new File("/sbin/au").exists() ) {
						exec = Runtime.getRuntime().exec("/sbin/au -c " + abspath);
					} else {
